            ...
            if (context.Node is ClassDeclarationSyntax classDeclarationSyntax)
            {
                ...
                if (useTraitAttributes.Any())
                {
                    ...
                    foreach (var useTraitAttribute in useTraitAttributes)
                    {
                        ...
                        var exposeMode = useTraitAttribute.GetNamedArgument("AutoExpose", Expose.None);

                        if (traitClass != null)
                        {
                            if (WorkItems[modifiedClass].TraitClasses.Add(new(traitClass, exposeMode)))
                                Log.Add($"\tAdding trait: {traitClass.FullName()} with auto-expose mode {exposeMode}");
                        }
                    }
                }
            }
